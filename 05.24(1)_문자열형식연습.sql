-- SQL : Structured Query Language	(구조화된 질의 언어)
	-- 1) DML : 데이터 추가, 삭제, 수정, 조회하는 명령들이 포함 (insert,delete,update,select)
	-- 2) DDL : 데이터를 저장할 테이블 또는 사용자 등등 오라클의 객체 생성/삭제하는 명령들이 포함 (create,alter,drop)
	-- 3) DCL : 계정 등등 데이터를 다루는데 필요한 기능을 실행하는 명령들이 포함 (grant,revoke)
...
-- VARCHAR2(길이) : 가변형길이 단위 바이트, 길이는 최대길이이고 실제로 메모리는 데이터크기만큼 사용합니다.
		-- 최대 2000바이트입니다. UTF-8 인코딩에서 한글은 3바이트(오라클)(자바는 2바이트), 영문/숫자/기호는 1바이트
-- 고정길이는 지정된 만큼 기억공간을 차지합니다.
-- 가변길이는 데이터 크기만큼 기억공간을 사용하고, 최대 크기로 제한합니다.
...
INSERT INTO TBL_STRING (ccol) VALUES ('가나');	-- 한글은 1글자가 3바이트
...
INSERT INTO TBL_STRING (dcol) VALUES ('가나');	-- 한글은 1글자가 3바이트
